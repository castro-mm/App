...
public class IdentityService : IIdentityService
{
    ...
    public async Task<string> LoginUserAsync(string email, string password)
    {
        ...
        if (!result)
            ValidationResult.AddError("CREDENCIAIS_INVALIDAS", "E-mail ou Senha incorretos");
    ...
    public async Task<string> LoginWithExternalProviderAsync(string provider, string providerKey, string? email, string? nomeCompleto)
    {
        ...
        if (user == null)
        {
            ...
            if (user == null)
            {
                ...
                {
                    UserName = email ?? $"{provider.ToLower()}_{providerKey}",
                    Email = email,
                    NomeCompleto = !string.IsNullOrWhiteSpace(nomeCompleto) ? nomeCompleto : "Usuário",
                    EmailConfirmed = true,
                    IsActive = true
                };
