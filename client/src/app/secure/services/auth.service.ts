...
export class AuthService {
    ...
    private _sessionExpiredByTimer = signal<boolean>(false);
    ...
    readonly sessionExpiredByTimer = this._sessionExpiredByTimer.asReadonly();
    ...
    async loginWithGoogle(idToken: string): Promise<ApiResponse> {
        const response$ = this.http.post<ApiResponse>(`${this.apiUrl}/google-login`, { idToken });
        const response = await firstValueFrom(response$);

        if (response.result?.isSuccessful && response.result.data?.token) {
            this.setToken(response.result.data.token, false);
            this.scheduleExpiryCheck();
            await this.loadCurrentUser();
        }
        return response;
    ...
    async logout(): Promise<void> {
        ...
        this._sessionExpiredByTimer.set(false);
    ...
    private scheduleExpiryCheck(): void {
        ...
        this.expiryTimeout = setTimeout(() => {
            localStorage.removeItem('breadcrumb');
            this._sessionExpiredByTimer.set(true);
        }, remaining);
    }
}
