...
export abstract class EntityListComponent<T extends Entity, TParams extends Params, TComponent> {
    ...
    async listar(): Promise<void> {
        ...
        await this.service.getByParams(params) // maybe just get()
            ...
            .catch((ex: any) => {
                const apiError = ex.error as ApiResponse;
                if (apiError?.statusCode !== StatusCode.NotFound) {
                    this.messageService.showMessageFromReponse(apiError);
                }
            })            
    ...
    /**
     * @summary Fornece sugestões para campos de autocomplete com base nos valores atuais da lista de entidades.
     * @param event Evento de autocomplete do primeng, contendo a query de busca.
     * @param field Campo da entidade para o qual as sugestões devem ser geradas.
     * @description Filtra os valores únicos do campo especificado na lista de entidades com base na query de busca.
     */
    filter(event: AutoCompleteCompleteEvent, field: string): void {
        const query = event.query.toLowerCase();
        this.autocompleteSuggestions[field] = [...new Set(
            this.entityList()
                .map(c => c[field as keyof T] as unknown as string)
